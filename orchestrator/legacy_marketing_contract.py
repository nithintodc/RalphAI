"""Maps insights to campaign_plan items — legacy flow_manager contract."""
...
def build_campaign_plan(operator_id: str, insights: list[str]) -> list[dict]:
    ...
    if "weekend" in text or "traffic" in text:
        plan.append({"type": "ads", "budget": 100, "target": "weekend traffic"})
    if "aov" in text or "order" in text:
        plan.append({"type": "offers", "discount": "20%", "condition": "above $30"})
    if not plan:
