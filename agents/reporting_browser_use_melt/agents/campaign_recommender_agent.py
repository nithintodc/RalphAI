...
from agents.campaign_params import STORE_ID_COL
...
def build_recommendations(store_aov: "pd.DataFrame") -> "pd.DataFrame":
    ...
    Args:
        store_aov: DataFrame with columns "Store ID" (or legacy "Merchant store ID" / "Merchant Store ID") and "AOV" (numeric).
    ...
    for candidate in (
        ...
        if candidate in store_aov.columns:
            store_col = candidate
            break
    if store_col is None:
        store_col = store_aov.columns[0]
    out = store_aov[[store_col, "AOV"]].copy()
    out = out.rename(columns={store_col: STORE_ID_COL})
