...
# Canonical DataFrame / Excel column for store identifier (DoorDash CSVs may use "Merchant store ID").
...
def normalize_merchant_store_id(val) -> str:
    """
    Canonical store id string for Excel/CSV (avoids 22978566.0 vs 22978566 lookup misses).

    Integer-like numbers become decimal-free strings; non-numeric ids pass through stripped.
    """
    if val is None:
        return ""
    if pd is not None:
...
def lookup_store_name(store_id_to_name: dict, store_id) -> str:
    """Resolve store name using normalized id and a plain-string fallback."""
    if not store_id_to_name:
        return ""
    k = normalize_merchant_store_id(store_id)
    if k and k in store_id_to_name:
        return str(store_id_to_name[k]).strip()
    s = str(store_id).strip() if store_id is not None else ""
    return str(store_id_to_name.get(s, "") or "").strip()
...
def _load_store_id_to_name(xl) -> dict:
    ...
    Looks for columns 'Store ID' (or legacy merchant store id headers) and 'Store Name' (header at row 3, 0-indexed row 2).
    ...
    for sheet_name in xl.sheet_names:
        if sheet_name.lower().replace("-", "").replace(" ", "") in ("storewise", "financialstorewise"):
            try:
                ...
                id_col = None
                for key in ("store id", "merchant store id", "shop id"):
                    if key in lowered:
                        id_col = lowered[key]
                        break
                name_col = lowered.get("store name")
                if id_col and name_col:
                    for _, row in df.dropna(subset=[id_col]).iterrows():
                        sid = normalize_merchant_store_id(row[id_col])
...
def get_store_ids_from_combined_analysis(combined_xlsx_path: Path) -> List[str]:
    ...
    for name in xl.sheet_names:
        ...
        if match:
            sid = normalize_merchant_store_id(match.group(1).strip())
        else:
            idx = name.find("Day-Slot - ")
            raw = (name[idx + len("Day-Slot - "):].strip() if idx >= 0 else "")
            sid = normalize_merchant_store_id(raw)
...
def get_subtotal_to_tags_per_store_from_combined(
    ...
    for sheet_name in xl.sheet_names:
        ...
        if match:
            store_id = normalize_merchant_store_id(match.group(1).strip())
        else:
            idx = sheet_name.find("Day-Slot - ")
            raw = (sheet_name[idx + len("Day-Slot - "):].strip() if idx >= 0 else "")
            store_id = normalize_merchant_store_id(raw)
        if not store_id:
            continue
...
def get_campaign_combos_from_slots_and_combined(
    ...
    for store_id, subtotal_to_tags in per_store.items():
        for min_subtotal, tags in subtotal_to_tags.items():
            ...
            store_name = lookup_store_name(store_id_to_name, store_id)
            if store_name:
