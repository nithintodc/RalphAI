...
CAMPAIGN_MAPPINGS_HEADERS = (
    STORE_ID_COL, "Store Name", "Minimum Subtotal", "Slot Tags", "Campaign Name", "Status")
...
def backfill_campaign_mapping_store_names(combined_xlsx_path: Path) -> bool:
    """
    For an existing combined analysis workbook: normalize Store ID cells (strip Excel .0),
    fill blank Store Name from the Store-wise sheet, and fix TODC-{id}.0-$ patterns in Campaign Name.
    """
    try:
        import openpyxl
        import pandas as pd
    ...
    from agents.campaign_params import _load_store_id_to_name, lookup_store_name
    ...
    try:
        ...
        logger.warning("CombinedReportAgent: backfill could not load store names: %s", e)
        store_id_to_name = {}

    try:
        wb = openpyxl.load_workbook(path, read_only=False, data_only=True)
    except Exception as e:
        logger.warning("CombinedReportAgent: backfill could not open %s: %s", path, e)
        return False
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=2):
            ...
            name_cell = row[1]
            camp_cell = row[4]
            cid = normalize_merchant_store_id(sid_cell.value)
            if cid and sid_cell.value != cid:
                sid_cell.value = cid
                updated += 1
            name_val = str(name_cell.value or "").strip()
            if not name_val and cid:
                filled = lookup_store_name(store_id_to_name, cid)
                if filled:
                    name_cell.value = filled
                    updated += 1
            cn = str(camp_cell.value or "").strip()
            if cn and ".0-" in cn:
                cn2 = _re.sub(r"-(\d+)\.0-", r"-\1-", cn)
                if cn2 != cn:
                    camp_cell.value = cn2
                    updated += 1
        if updated:
            wb.save(path)
            logger.info("CombinedReportAgent: backfilled %s cell(s) in Campaign Mappings (%s)", updated, path.name)
        return True
    except Exception as e:
        logger.warning("CombinedReportAgent: backfill failed: %s", e)
        return False
    finally:
        wb.close()
...
def append_campaign_mappings_to_workbook(
    ...
    for row_idx, m in enumerate(mappings, start=2):
        store_id = normalize_merchant_store_id(m.get("store_id", ""))
    ...
    try:
        ...
        for row_idx, row in enumerate(ws.iter_rows(values_only=True), start=1):
            ...
            if not row or len(row) < 5:
                continue
            store_id = normalize_merchant_store_id(row[0] or "")
