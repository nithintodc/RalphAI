...
{s + 1}. LOCATIONS: keep default "All stores", click "Next".
{s + 2}. Choose "By date range". Set Start date: {start_date}, End date: {end_date}. Click "Create report". WAIT UNTIL the report appears.

=== STEP {s + 3}: Download the Financial Report IMMEDIATELY ===
{s + 3}. Click the DOWNLOAD icon on the TOPMOST new Financial report row. WAIT UNTIL the download completes.

=== STEP {s + 4}: Generate Marketing Report ===
{s + 4}. Click "Create report". Select "Marketing report" RADIO BUTTON, click "Next".
{s + 5}. UNCHECK "Online Ordering". Keep "Marketplace" CHECKED. Click "Next".
...
{s + 8}. Click the DOWNLOAD icon on the TOPMOST new Marketing report row. WAIT UNTIL the download completes.
...
def get_task_description_campaign_for_subtotal_combo(combo: dict) -> str:
    ...
    # DoorDash "Choose stores" search matches names; IDs typed as floats (e.g. 22978566.0) often fail.
    ...
    if store_name:
        ...
        store_search_primary = (
            f"the exact location name as shown in the store list (internal id {store_id_digits or store_id} is for your reference only — do not type .0 or search the id first)"
        )
