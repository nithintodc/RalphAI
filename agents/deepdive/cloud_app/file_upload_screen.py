...
.mapping-rule .count {
    ...
    if 'manual_cloud_uris' not in st.session_state:
        st.session_state.manual_cloud_uris = {
            "dd_financial_uri": "",
            "ue_financial_uri": "",
            "marketing_uri": "",
        }
    ...
    if st.button("Run Using App", type="primary", disabled=not dates_provided, use_container_width=True):
        valid = True
    ...
    def _pick_latest_match(items, keywords):
        matched = [item for item in items if all(keyword in item.lower() for keyword in keywords)]
        if not matched:
            return ""
        return sorted(matched)[-1]

    if st.button("Run Using Cloud", type="secondary", disabled=not dates_provided, use_container_width=True):
        if not cloud_config.enabled:
            st.error("Cloud run is not configured for this app.")
            return
        ...
        try:
            dd_local = st.session_state.get("uploaded_dd_data")
            ue_local = st.session_state.get("uploaded_ue_data")
            marketing_local = st.session_state.get("uploaded_marketing_folder")

            if dd_local and Path(dd_local).exists() and ue_local and Path(ue_local).exists():
                upload_job_id = new_job_id("upload")
                dd_dest = build_upload_object_path(upload_job_id, "dd_financial", Path(dd_local).name)
                ue_dest = build_upload_object_path(upload_job_id, "ue_financial", Path(ue_local).name)
                dd_uri = upload_local_file_to_gcs(Path(dd_local), cloud_config.bucket, dd_dest)
                ue_uri = upload_local_file_to_gcs(Path(ue_local), cloud_config.bucket, ue_dest)

                if marketing_local and Path(marketing_local).exists():
                    marketing_dest = f"uploads/{upload_job_id}/marketing_bundle/"
                    any_marketing = False
                    for file_path in Path(marketing_local).rglob("*.csv"):
                        relative = file_path.relative_to(Path(marketing_local))
                        gcs_path = f"{marketing_dest}{relative.as_posix()}"
                        upload_local_file_to_gcs(file_path, cloud_config.bucket, gcs_path)
                        any_marketing = True
                    if any_marketing:
                        marketing_uri = f"gs://{cloud_config.bucket}/{marketing_dest}"
            else:
                objects = list_gcs_objects(cloud_config.bucket, prefix="uploads/", suffix=".csv")
                dd_uri = _pick_latest_match(objects, ["dd"]) or _pick_latest_match(objects, ["doordash"]) or _pick_latest_match(objects, ["financial"])
                ue_uri = _pick_latest_match(objects, ["ue"]) or _pick_latest_match(objects, ["uber"])

            if not dd_uri or not ue_uri:
                st.error("Could not prepare cloud files automatically. Add files in app first.")
                return
            ...
            st.success("Cloud run started.")
        except Exception as exc:
            st.error(f"Cloud run failed to start: {exc}")

    latest = st.session_state.get("cloud_last_submission")
    if latest and st.button("Refresh Cloud Status", use_container_width=True):
        try:
            status = get_cloud_run_operation_status(cloud_config, latest.get("operation_name", ""))
            done = status.get("done", False)
            if status.get("error"):
                st.error("Cloud run failed.")
            elif done:
                st.success("Cloud run finished.")
            else:
                st.info("Cloud run is still processing.")
        except Exception:
            st.warning("Could not refresh status right now.")
