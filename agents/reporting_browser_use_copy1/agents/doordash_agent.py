...
REPORTS RULE (critical): For financial and marketing reports, never change store/location selection. Go straight to date range → Create report → download. Do not use find_elements/evaluate/search_page to hunt for stores.
...
7. On "Customize your Financial report": SKIP store/location selection entirely. Do NOT scroll hunting for stores, "Select All", checkboxes, or click "Stores"/"1 store" in the summary panel (that text is read-only). Accept whatever store count the portal shows by default.
8. Find "By date range" (or the date-range option) on this page. Set Start date: {start_date}, End date: {end_date}. If you must click "Next" once to reach the date fields, do that without changing stores. Click "Create report". WAIT UNTIL the report appears in the list (it may take several seconds to generate).
...
12. On the customize page: SKIP store/location selection entirely (same as financial report — do not change stores or hunt for "Select All").
