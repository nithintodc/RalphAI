...
def _combine_promo_and_sponsored(promo_tbl, sponsored_tbl):
    """Merge promotion + sponsored tables by summing values per campaign type."""
    if promo_tbl.empty and sponsored_tbl.empty:
        return pd.DataFrame()
    if promo_tbl.empty:
        return sponsored_tbl.copy()
    if sponsored_tbl.empty:
        return promo_tbl.copy()

    all_indices = set(promo_tbl.index) | set(sponsored_tbl.index)
    combined_data = []
    for idx in all_indices:
        promo_row = promo_tbl.loc[idx] if idx in promo_tbl.index else pd.Series({
            'Orders': 0, 'Sales': 0, 'Spend': 0, 'ROAS': 0, 'Cost per Order': 0
        })
        sponsored_row = sponsored_tbl.loc[idx] if idx in sponsored_tbl.index else pd.Series({
            'Orders': 0, 'Sales': 0, 'Spend': 0, 'ROAS': 0, 'Cost per Order': 0
        })
        combined_row = {
    ...
    result = pd.DataFrame(combined_data, index=list(all_indices))
    return result[['Orders', 'Sales', 'Spend', 'ROAS', 'Cost per Order']]


def _process_marketing_for_date_range(start_date, end_date, excluded_dates=None, marketing_folder_path=None):
    """Process marketing files for a specific date range. Returns (promo, sponsored, combined)."""
    promo = process_marketing_promotion_files(
        excluded_dates=excluded_dates,
        post_start_date=start_date,
        post_end_date=end_date,
        marketing_folder_path=marketing_folder_path,
    )
    sponsored = process_marketing_sponsored_files(
        excluded_dates=excluded_dates,
        post_start_date=start_date,
        post_end_date=end_date,
        marketing_folder_path=marketing_folder_path,
    )
    combined = _combine_promo_and_sponsored(promo, sponsored)
    return promo, sponsored, combined
...
def create_corporate_vs_todc_table(excluded_dates=None, pre_start_date=None, pre_end_date=None, post_start_date=None, post_end_date=None, marketing_folder_path=None):
    ...
    Returns:
        ...
        These are the POST-period tables (backward-compatible).
    """
    ...
    combined_table = _combine_promo_and_sponsored(promotion_table, sponsored_table)
    return promotion_table, sponsored_table, combined_table


def create_corporate_vs_todc_all_periods(excluded_dates=None, pre_start_date=None, pre_end_date=None, post_start_date=None, post_end_date=None, marketing_folder_path=None):
    """
    Create Corporate vs TODC tables for ALL periods: Pre, Post, LY Pre, LY Post.

    Returns dict of {period_label: (promo_table, sponsored_table, combined_table)}.
    """
    from data_processing import get_last_year_dates

    results = {}
    if pre_start_date and pre_end_date:
        results['Pre'] = _process_marketing_for_date_range(
            pre_start_date, pre_end_date, excluded_dates, marketing_folder_path
        )
    if post_start_date and post_end_date:
        results['Post'] = _process_marketing_for_date_range(
            post_start_date, post_end_date, excluded_dates, marketing_folder_path
        )

    ly_pre_start, ly_pre_end = get_last_year_dates(pre_start_date, pre_end_date)
    if ly_pre_start and ly_pre_end:
        results['LY Pre'] = _process_marketing_for_date_range(
            ly_pre_start, ly_pre_end, excluded_dates, marketing_folder_path
        )

    ly_post_start, ly_post_end = get_last_year_dates(post_start_date, post_end_date)
    if ly_post_start and ly_post_end:
        results['LY Post'] = _process_marketing_for_date_range(
            ly_post_start, ly_post_end, excluded_dates, marketing_folder_path
        )
