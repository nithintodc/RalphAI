...
from app_design import render_button_group, style_signed_table


def _unique_ue_row_labels(names: pd.Series, sids: pd.Series) -> pd.Series:
    """Build unique row labels from Store Name, falling back to Store ID; disambiguate duplicate names."""
    names = names.astype(str).replace("nan", "").str.strip()
    sids = sids.astype(str)
    labels = names.where(names != "", sids)
    dup = labels.duplicated(keep=False)
    suffix = " (" + sids + ")"
    return labels.where(~dup, labels.astype(str) + suffix)


def _uber_eats_sidebar_labels(df, all_stores) -> tuple[dict, dict]:
    """Maps Store ID → human label using Store Name; duplicate shared names append (Store ID)."""
    raw_by_sid = {}
    for sid in all_stores:
        sub = df.loc[df["Store ID"].astype(str) == str(sid), "Store Name"]
        raw = sub.iloc[0] if len(sub) else None
        raw_by_sid[sid] = "" if raw is None or pd.isna(raw) else str(raw).strip()

    name_counts = Counter(v for v in raw_by_sid.values() if v)

    sid_to_label = {}
    for sid in all_stores:
        nm = raw_by_sid[sid]
        if not nm:
            sid_to_label[sid] = str(sid)
        elif name_counts[nm] > 1:
            sid_to_label[sid] = f"{nm} ({sid})"
        else:
            sid_to_label[sid] = nm
    label_to_sid = {sid_to_label[s]: s for s in all_stores}
    return sid_to_label, label_to_sid

def _store_tables_use_name_index(df, platform_name=None):
    """Use Store Name as the row index when appropriate; UE uses Store Name (not numeric Store ID index)."""
    if df is None or df.empty:
        return df
    out = df.copy()

    if platform_name == "UberEats" and "Store Name" in out.columns and "Store ID" in out.columns:
        names = out["Store Name"].astype(str).replace("nan", "").str.strip()
        sid = out["Store ID"].astype(str)
        row_labels = _unique_ue_row_labels(names, sid)
        out = out.drop(columns=["Store Name", "Store ID"])
        return out.set_index(row_labels).rename_axis("Store Name")

    if "Store Name" not in out.columns or "Store ID" not in out.columns:
        if "Store ID" in out.columns:
            return out.set_index("Store ID")
        return out
    names = out["Store Name"].astype(str).replace("nan", "").str.strip()
    sid = out["Store ID"].astype(str)
    out["_row_label"] = _unique_ue_row_labels(names, sid)
    out = out.drop(columns=["Store Name", "Store ID"])
    return out.set_index("_row_label").rename_axis("Store Name")
...
def create_store_selector(platform_name, df, platform_key, file_uploaded=False, date_ranges_set=False):
    ...
    with st.expander(f"{platform_name} Store Selection", expanded=True):
        ...
        all_stores = sorted(df['Store ID'].unique().tolist(), key=lambda x: str(x))

        use_ue_labels = platform_name == 'UberEats' and 'Store Name' in df.columns
        sid_to_label = {}
        label_to_sid = {}
        if use_ue_labels:
            sid_to_label, label_to_sid = _uber_eats_sidebar_labels(df, all_stores)
            option_labels = [sid_to_label[s] for s in all_stores]
        ...
        group_clicks = render_button_group(
            [
                {"label": "Select All", "key": f"select_all_{platform_name}"},
                {"label": "Clear", "key": f"deselect_all_{platform_name}"},
            ]
        )
        if group_clicks.get(f"select_all_{platform_name}"):
            st.session_state[platform_key] = all_stores.copy()
            st.rerun()
        if group_clicks.get(f"deselect_all_{platform_name}"):
            st.session_state[platform_key] = []
            st.rerun()

        # Multi-select for stores (Uber Eats: friendly labels, session keeps Store IDs)
        if use_ue_labels:
            default_labels = [sid_to_label[s] for s in default_stores if s in sid_to_label]
            ...
            if render_button_group([{"label": "Apply Selection", "key": f"apply_{platform_name}", "primary": True}]).get(
                f"apply_{platform_name}"
            ):
                st.session_state[platform_key] = [label_to_sid[l] for l in selected_labels]
                st.rerun()
        else:
...
def display_store_tables(platform_name, table1_df, table2_df):
    ...
    with col_left:
        ...
        if not table1_display.empty:
            ...
            table1_display = _store_tables_use_name_index(table1_display, platform_name)
            st.dataframe(style_signed_table(table1_display), width='stretch', height=290)
        else:
    ...
    with col_right:
        if table2_df is not None and not table2_df.empty:
            ...
            if not table2_display.empty:
                ...
                if 'YoY%' in table2_display.columns:
                    table2_display['YoY%'] = table2_display['YoY%'].apply(lambda x: f"{x:.1f}%")
                table2_display = _store_tables_use_name_index(table2_display, platform_name)
                st.dataframe(style_signed_table(table2_display), width='stretch', height=290)
            else:
...
def display_platform_data(platform_name, sales_df, payouts_df, sales_label, platform_key):
    ...
    if platform_name == 'UberEats' and 'Store Name' in sales_df.columns:
        st.caption(f"All values represent aggregated sums of **{sales_label}** by store")
    else:
    ...
    t1_cols = ['Store ID', 'pre_25', 'post_25', 'PrevsPost', 'LastYear_Pre_vs_Post', 'Growth%']
    if platform_name == 'UberEats' and 'Store Name' in filtered_sales_df.columns:
        t1_cols.insert(1, 'Store Name')
    table1_df = filtered_sales_df[[c for c in t1_cols if c in filtered_sales_df.columns]].copy()
    ...
    st.subheader("Table 2: Year-over-Year Analysis")
    # Create table for CSV (keep numeric values)
    t2_cols = ['Store ID', 'post_24', 'post_25', 'YoY', 'YoY%']
    if platform_name == 'UberEats' and 'Store Name' in filtered_sales_df.columns:
        t2_cols.insert(1, 'Store Name')
    table2_df = filtered_sales_df[[c for c in t2_cols if c in filtered_sales_df.columns]].copy()
