...
def create_combined_store_tables(dd_table1, dd_table2, ue_table1, ue_table2):
    ...
    if dd_table1 is not None and ue_table1 is not None:
        ...
        combined_table1 = _coalesce_store_name_after_merge(combined_table1)
        ...
        # Keep only the needed columns (optional Store Name from UE)
        t1_cols = ['Store ID', 'Pre', 'Post', 'PrevsPost', 'LastYear Pre vs Post', 'Growth%']
        if 'Store Name' in combined_table1.columns:
            t1_cols.insert(1, 'Store Name')
        combined_table1 = combined_table1[[c for c in t1_cols if c in combined_table1.columns]]
    ...
    if dd_table2 is not None and ue_table2 is not None:
        ...
        combined_table2 = _coalesce_store_name_after_merge(combined_table2)
        # Sum numeric columns for stores in both platforms
        ...
        # Keep only the needed columns (optional Store Name from UE)
        t2_cols = ['Store ID', 'last year-post', 'post', 'YoY', 'YoY%']
        if 'Store Name' in combined_table2.columns:
            t2_cols.insert(1, 'Store Name')
        combined_table2 = combined_table2[[c for c in t2_cols if c in combined_table2.columns]]
...
def _coalesce_store_name_after_merge(df):
    """After merging DD + UE rows on Store ID, keep a single Store Name column when present."""
    ...
    ue_c = 'Store Name_ue'
    dd_c = 'Store Name_dd'
    if ue_c in out.columns and dd_c in out.columns:
        out = out.copy()
        out['Store Name'] = out[ue_c].combine_first(out[dd_c])
        return out.drop(columns=[dd_c, ue_c])
    if ue_c in out.columns:
        return out.rename(columns={ue_c: 'Store Name'})
    if dd_c in out.columns:
        return out.rename(columns={dd_c: 'Store Name'})
    return out
...
def get_platform_store_tables(sales_df, platform_key):
    ...
    use_ue_store_name = platform_key == 'selected_stores_UberEats' and 'Store Name' in filtered_sales_df.columns
    
    # Table 1
    t1_src = ['Store ID', 'pre_25', 'post_25', 'PrevsPost', 'LastYear_Pre_vs_Post', 'Growth%']
    if use_ue_store_name:
        t1_src = ['Store Name', 'Store ID'] + [c for c in t1_src if c != 'Store ID']
    table1_df = filtered_sales_df[[c for c in t1_src if c in filtered_sales_df.columns]].copy()
    ...
    # Table 2 (YoY)
    t2_src = ['Store ID', 'post_24', 'post_25', 'YoY', 'YoY%']
    if use_ue_store_name:
        t2_src = ['Store Name', 'Store ID'] + [c for c in t2_src if c != 'Store ID']
    table2_df = filtered_sales_df[[c for c in t2_src if c in filtered_sales_df.columns]].copy()
