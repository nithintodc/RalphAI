...
from utils import normalize_store_id_column, normalize_ue_store_key_column, filter_excluded_dates
...
def load_and_aggregate_new_customers(excluded_dates=None, pre_start_date=None, pre_end_date=None, 
    ...
    def process_ue_mkt_file(file_path, excluded_dates=None):
        ...
        try:
            ...
            # Prefer Shop ID over repeating chain Store IDs (same as ue-data master path)
            df, store_col = normalize_ue_store_key_column(df)

            # Check if there's a Store ID / Shop ID-derived key in the file
...
def _collect_ue_store_name_map(*dfs):
    """Build one Store ID -> Store Name map from UE aggregates (avoids merge duplicate columns)."""
    parts = []
    for df in dfs:
        if df is not None and not df.empty and 'Store Name' in df.columns and 'Store ID' in df.columns:
            parts.append(df[['Store ID', 'Store Name']].copy())
    if not parts:
        return None
    m = pd.concat(parts, ignore_index=True)
    m['Store ID'] = m['Store ID'].astype(str)
    return m.drop_duplicates(subset=['Store ID'], keep='first')


def _drop_ue_store_name_column(df):
    if df is None or df.empty or 'Store Name' not in df.columns:
        return df
    return df.drop(columns=['Store Name'])


def _keep_store_id_and_period(df, period_col):
    """Keep only Store ID and the period metric column for outer merges."""
    if df is None or df.empty:
        return df
    keep = [c for c in ('Store ID', period_col) if c in df.columns]
    return df[keep]
...
def process_data(pre_24_sales, pre_24_payouts, pre_24_orders, post_24_sales, post_24_payouts, post_24_orders,
    ...
    ue_store_name_map = _collect_ue_store_name_map(pre_24_s, post_24_s, pre_25_s, post_25_s)
    pre_24_s = _drop_ue_store_name_column(pre_24_s)
    post_24_s = _drop_ue_store_name_column(post_24_s)
    pre_25_s = _drop_ue_store_name_column(pre_25_s)
    post_25_s = _drop_ue_store_name_column(post_25_s)
    ...
    if ue_store_name_map is not None and not sales_result.empty and 'Store ID' in sales_result.columns:
        sales_result['Store ID'] = sales_result['Store ID'].astype(str)
        sales_result = sales_result.merge(ue_store_name_map, on='Store ID', how='left')
    ...
    pre_24_p = _drop_ue_store_name_column(pre_24_p)
    post_24_p = _drop_ue_store_name_column(post_24_p)
    pre_25_p = _drop_ue_store_name_column(pre_25_p)
    post_25_p = _drop_ue_store_name_column(post_25_p)
    ...
    if ue_store_name_map is not None and not payouts_result.empty and 'Store ID' in payouts_result.columns:
        payouts_result['Store ID'] = payouts_result['Store ID'].astype(str)
        payouts_result = payouts_result.merge(ue_store_name_map, on='Store ID', how='left')
    ...
    pre_24_o = _keep_store_id_and_period(pre_24_o, 'pre_24')
    post_24_o = _keep_store_id_and_period(post_24_o, 'post_24')
    pre_25_o = _keep_store_id_and_period(pre_25_o, 'pre_25')
    post_25_o = _keep_store_id_and_period(post_25_o, 'post_25')
    ...
    pre_24_o = _drop_ue_store_name_column(pre_24_o)
    post_24_o = _drop_ue_store_name_column(post_24_o)
    pre_25_o = _drop_ue_store_name_column(pre_25_o)
    post_25_o = _drop_ue_store_name_column(post_25_o)
    ...
    if ue_store_name_map is not None and not orders_result.empty and 'Store ID' in orders_result.columns:
        orders_result['Store ID'] = orders_result['Store ID'].astype(str)
        orders_result = orders_result.merge(ue_store_name_map, on='Store ID', how='left')
