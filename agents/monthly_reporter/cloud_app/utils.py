...
def parse_datetime_column(series: pd.Series) -> pd.Series:
    """
    Parse a column of date strings that may mix MM/DD/YYYY, YYYY-MM-DD, etc.
    Tries explicit formats first so pandas does not emit UserWarning about
    inferring format / parsing element-by-element.
    """
    if series.empty:
    ...
    if still_na.any():
        parsed2 = pd.to_datetime(s.loc[still_na], format="%Y-%m-%d", errors="coerce")
        result.loc[still_na] = parsed2
    still_na = result.isna()
    if still_na.any():
        parsed3 = pd.to_datetime(s.loc[still_na], format="%m-%d-%Y", errors="coerce")
        result.loc[still_na] = parsed3
    still_na = result.isna()
    if still_na.any():
...
def filter_master_file_by_date_range(file_path, start_date, end_date, date_col_name, excluded_dates=None):
    ...
    try:
        ...
        if is_ue_file:
            # UberEats: MM/DD/YYYY first; then other common shapes without infer warnings
            ...
            if df[actual_date_col].isna().any():
                ...
                df.loc[mask_na, actual_date_col] = parse_datetime_column(original_dates.loc[mask_na])
        else:
            # DoorDash: mixed exports — explicit formats first, last resort suppressed
            df[actual_date_col] = parse_datetime_column(original_dates)
