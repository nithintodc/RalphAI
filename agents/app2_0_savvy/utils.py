...
def find_dd_store_name_column(df):
    """Detect DoorDash financial export column used as human-readable store name."""
...
def find_store_name_column(df):
    """Find the best store-name column across DD, UE, or marketing exports."""
    return find_dd_store_name_column(df) or find_ue_store_name_column(df)


def build_canonical_store_name_series(df, platform="auto"):
    """Build one store label per row; prefer human-readable names over numeric IDs."""
    if df is None or df.empty:
        return pd.Series(dtype="object")
    names = pd.Series([""] * len(df), index=df.index, dtype="object")
    name_col = find_store_name_column(df)
    if name_col and name_col in df.columns:
        names = _strip_blank_string_series(df[name_col])

    id_fallback_cols = []
    if platform in ("ue", "auto"):
        id_fallback_cols.extend(["Store ID", "Shop ID"])
    if platform in ("dd", "auto"):
        id_fallback_cols.extend(["Store ID", "Merchant store ID", "Merchant Store ID"])
    for id_col in id_fallback_cols:
        if id_col not in df.columns:
            continue
        ids = _strip_blank_string_series(df[id_col])
        if platform in ("ue", "auto") and id_col == "Store ID":
            ids = ids.map(coerce_ue_numeric_external_store_id)
        names = names.where(names.ne(""), ids.astype(str).str.strip())

    names = names.replace({"nan": "", "None": ""}).astype(str).str.strip()
    names = names.mask(names.eq("") | names.str.lower().isin(("nan", "none")), "Unknown Store")
    return names


def attach_store_name_column(df, platform="auto", inplace=False):
    """Attach canonical ``Store Name`` column for grouping across the app."""
    if df is None or df.empty:
        return df
    out = df if inplace else df.copy()
    out[STORE_NAME_COL] = build_canonical_store_name_series(out, platform=platform)
    return out


def get_dd_financial_store_name_column(df):
    """Return the raw DD store-name source column, if present."""
    return find_dd_store_name_column(df)
...
def finalize_ue_canonical_store_id_column(df: pd.DataFrame) -> pd.DataFrame:
    """Attach canonical ``Store Name`` for Uber Eats rows (names preferred over external IDs)."""
    if df is None or df.empty:
        return df
    return attach_store_name_column(df, platform="ue")
