...
def _build_dd_financial_breakdown(dd_data_path, start_date, end_date, excluded_dates,
    ...
    Columns = Store Names + Total  (each with Value and %)
    ...
    try:
        ...
        filtered = attach_store_name_column(filtered, platform="dd")
        store_col_name = STORE_NAME_COL
...
def _get_dd_store_col_name(df):
    """Find the DD store name column in a DataFrame."""
    if df is None or df.empty:
        return None
    tmp = attach_store_name_column(df, platform="dd")
    return STORE_NAME_COL if STORE_NAME_COL in tmp.columns else get_dd_financial_store_name_column(df)
...
def _filter_df_by_store_names(df, store_names, platform):
    if df is None or df.empty or not store_names:
        return df
    out = attach_store_name_column(df.copy(), platform=platform)
    selected = {str(s).strip() for s in store_names if str(s).strip()}
    return out[out[STORE_NAME_COL].astype(str).isin(selected)]
...
def _compute_window_metrics(
    ...
    """Compute financial metrics for one time window, optionally filtered by store."""
    if dd_df is None:
        dd_df = pd.DataFrame()
    if ue_df is None:
        ue_df = pd.DataFrame()

    dd_df = _filter_dd_order_rows(dd_df)

    if store_name is not None:
        if not dd_df.empty:
            dd_df = attach_store_name_column(dd_df, platform="dd")
            dd_df = dd_df[dd_df[STORE_NAME_COL].astype(str) == str(store_name)]
        if not ue_df.empty:
            ue_df = attach_store_name_column(ue_df, platform="ue")
            ue_df = ue_df[ue_df[STORE_NAME_COL].astype(str) == str(store_name)]
    else:
        dd_df = _filter_df_by_store_names(dd_df, dd_store_names, "dd")
        ue_df = _filter_df_by_store_names(ue_df, ue_store_names, "ue")
...
def build_financial_summary_table(dd_data_path, ue_data_path,
                                    ...
                                    dd_store_names=None, ue_store_names=None):
    """
    Build the unified financial summary table (aggregate or per-store).

    When dd_store_names / ue_store_names are provided, totals match dashboard
    summary tables for the same selected stores. DD Sales/Payouts use Order rows only.
    """
    if not all([pre_start, pre_end, post_start, post_end]):
        return pd.DataFrame()
    w = _load_all_windows(dd_data_path, ue_data_path, pre_start, pre_end,
                           post_start, post_end, excluded_dates)
    kw = {"store_name": store_id}
    if store_id is None:
        kw = {"dd_store_names": dd_store_names, "ue_store_names": ue_store_names}
    pre_m = _compute_window_metrics(w['dd_pre'], w['ue_pre'], **kw)
    post_m = _compute_window_metrics(w['dd_post'], w['ue_post'], **kw)
    ly_pre_m = _compute_window_metrics(w['dd_ly_pre'], w['ue_ly_pre'], **kw)
    ly_post_m = _compute_window_metrics(w['dd_ly_post'], w['ue_ly_post'], **kw)
...
def build_all_store_financial_tables(dd_data_path, ue_data_path,
    ...
    for key in ('dd_pre', 'dd_post', 'dd_ly_pre', 'dd_ly_post'):
        if not w[key].empty:
            dd_tmp = attach_store_name_column(w[key].copy(), platform="dd")
            if STORE_NAME_COL in dd_tmp.columns:
                all_ids.update(dd_tmp[STORE_NAME_COL].dropna().astype(str).unique())
    for key in ('ue_pre', 'ue_post', 'ue_ly_pre', 'ue_ly_post'):
        if not w[key].empty:
            ue_tmp = attach_store_name_column(w[key].copy(), platform="ue")
            if STORE_NAME_COL in ue_tmp.columns:
                all_ids.update(ue_tmp[STORE_NAME_COL].dropna().astype(str).unique())
    results = []
...
def _extract_period_table(tbl, prefix):
    """Split a marketing table into Pre-only or Post-only columns (matches dashboard UI)."""
...
def _prepare_corporate_campaign_export(tbl):
    """Label Corporate/TODC rows for Excel export."""
    if tbl is None or tbl.empty:
        return tbl
    out = tbl.copy()
    if out.index.name != "Campaign":
        out.index.name = "Campaign"
    out = out.reset_index()
    out["Campaign"] = out["Campaign"].apply(
        lambda x: "Corporate" if x is False else ("TODC" if x is True else str(x))
    )
    return out.set_index("Campaign")


def _merge_workbook_sheets(target_wb, source_wb, *, skip_sheets=None, sheet_prefix="Bkt "):
    """Copy sheets from *source_wb* into *target_wb*, skipping names in *skip_sheets*."""
    skip_sheets = skip_sheets or set()
    for name in source_wb.sheetnames:
        if name in skip_sheets:
            continue
        src = source_wb[name]
        dest_name = f"{sheet_prefix}{name}"[:31]
        base = dest_name
        n = 1
        while dest_name in target_wb.sheetnames:
            suffix = f"_{n}"
            dest_name = f"{base[: 31 - len(suffix)]}{suffix}"
            n += 1
        dest = target_wb.create_sheet(dest_name)
...
def export_to_excel(dd_table1, dd_table2, ue_table1, ue_table2, 
                    ...
                    campaign_metrics_table=None,
                    dd_campaign_tables=None,
                    ue_campaign_pivots=None,
    ...
    if dd_sales_df is not None and not dd_sales_df.empty and dd_payouts_df is not None and dd_orders_df is not None:
    ...
    if ue_sales_df is not None and not ue_sales_df.empty and ue_payouts_df is not None and ue_orders_df is not None:
    ...
    def add_table_to_sheet(ws, table_name, df, start_row, start_col=1):
        ...
        start_row += 1
        # Add table data
        # Reset index for named indices so key labels (Campaign, Campaign name, etc.) are exported.
        if df.index.name:
        ...
        for row_idx, row_data in df_display.iterrows():
            ...
            for col_idx, col_name in enumerate(df_display.columns, start=1):
                ...
                elif col_name in ['Store ID', 'Metric', 'Campaign', 'Campaign name', 'Is Self Serve Campaign', 'Merchant Store IDs', 'Markups',
                                  'Source', 'Period', 'Timezone', 'Budget unit', 'Status', 'Audience', 'Offer type']:
                    # Keep as is (text)
                    pass
                ...
                elif col_name in ['Impressions', 'Clicks']:
                    if isinstance(value, (int, float)):
                        cell.value = f"{int(round(value)):,}"
                elif col_name in ['Sales', 'Spend', 'Cost per Order', 'Campaign-AOV', 'Sales / Orders', 'Check after promo', 'Check after promotion']:
                    # Sales, Spend, Cost per Order: format as dollar amount
                    if isinstance(value, (int, float)):
                        cell.value = f"${value:,.2f}"
                elif col_name.startswith('Orders '):
                    if isinstance(value, (int, float)):
                        cell.value = f"{int(round(value)):,}"
                elif col_name.startswith(('Sales ', 'Payouts ')) and 'Growth%' not in col_name and 'YoY%' not in col_name:
                    if isinstance(value, (int, float)):
                        cell.value = f"${value:,.1f}"
                elif col_name in ['ROAS', 'CTR', 'Click to Order']:
                    # Rates: decimal
                    if isinstance(value, (int, float)):
                        cell.value = f"{value:.2f}"
    ...
    if corporate_todc_table is not None and not corporate_todc_table.empty:
        ...
        corporate_export = _prepare_corporate_campaign_export(corporate_todc_table)
        current_row = add_table_to_sheet(ws_corporate, "Combined: Corporate vs TODC", corporate_export, current_row)

        for period in ("Pre", "Post"):
            period_tbl = _extract_period_table(corporate_todc_table, period)
            if not period_tbl.empty:
                current_row = add_table_to_sheet(
                    ws_corporate,
                    f"Corp vs TODC - {period}",
                    _prepare_corporate_campaign_export(period_tbl),
                    current_row,
                )
        
        # Add Promotion table if available
        if promotion_table is not None and not promotion_table.empty:
            promo_export = _prepare_corporate_campaign_export(promotion_table)
            current_row = add_table_to_sheet(ws_corporate, "Promotion: Corporate vs TODC", promo_export, current_row)
            for period in ("Pre", "Post"):
                period_tbl = _extract_period_table(promotion_table, period)
                if not period_tbl.empty:
                    current_row = add_table_to_sheet(
                        ws_corporate,
                        f"Promotion: Corporate vs TODC - {period}",
                        _prepare_corporate_campaign_export(period_tbl),
                        current_row,
                    )
        ...
        if sponsored_table is not None and not sponsored_table.empty:
            sponsored_export = _prepare_corporate_campaign_export(sponsored_table)
            current_row = add_table_to_sheet(ws_corporate, "Sponsored Listing: Corporate vs TODC", sponsored_export, current_row)
            for period in ("Pre", "Post"):
                period_tbl = _extract_period_table(sponsored_table, period)
                if not period_tbl.empty:
                    current_row = add_table_to_sheet(
                        ws_corporate,
                        f"Sponsored Listing: Corporate vs TODC - {period}",
                        _prepare_corporate_campaign_export(period_tbl),
                        current_row,
                    )

        # Add campaign-period metrics table if available
        if campaign_metrics_table is not None and not campaign_metrics_table.empty:
            campaign_export = campaign_metrics_table.copy()
            current_row = add_table_to_sheet(
                ws_corporate,
                "Campaign Metrics: Sales/Spend/ROAS/CPO/Sales per Order/Check after promo",
                campaign_export,
                current_row
            )

        if dd_campaign_tables:
            for key, title in [
                ("promo_pre", "DD Promo Campaign Names - Pre"),
                ("promo_post", "DD Promo Campaign Names - Post"),
                ("ads_pre", "DD Ads Campaign Names - Pre"),
                ("ads_post", "DD Ads Campaign Names - Post"),
            ]:
                tbl = dd_campaign_tables.get(key)
                if tbl is not None and not tbl.empty:
                    current_row = add_table_to_sheet(ws_corporate, title, tbl, current_row)

        if ue_campaign_pivots:
            for key, title in [
                ("campaign_pre_post", "UE Ads: Campaign Pre vs Post"),
                ("combined_by_period", "UE Ads: Combined by Period"),
                ("store_pre_post", "UE Ads: Store-wise Pre vs Post"),
                ("offer_type", "UE Pivot: Campaign Name"),
                ("audience", "UE Pivot: Audience"),
                ("status", "UE Pivot: Status"),
                ("timezone", "UE Pivot: Timezone"),
                ("budget_unit", "UE Pivot: Budget Unit"),
            ]:
                piv = ue_campaign_pivots.get(key)
                if piv is not None and not piv.empty:
                    current_row = add_table_to_sheet(ws_corporate, title, piv, current_row)
            for file_key, piv in (ue_campaign_pivots.get("file_tables", {}) or {}).items():
                if piv is not None and not piv.empty:
                    current_row = add_table_to_sheet(ws_corporate, f"UE Campaign Table: {file_key}", piv, current_row)
    ...
    for label, tbl in [("Combined", combined_store_table1), ("DoorDash", dd_table1), ("UberEats", ue_table1)]:
        if tbl is not None and not tbl.empty:
            ...
            id_col = STORE_NAME_COL if STORE_NAME_COL in df_t.columns else (
                'Store ID' if 'Store ID' in df_t.columns else (df_t.columns[0] if len(df_t.columns) > 0 else None)
            )
            pre_col = 'Sales Pre' if 'Sales Pre' in df_t.columns else ('Pre' if 'Pre' in df_t.columns else None)
            post_col = 'Sales Post' if 'Sales Post' in df_t.columns else ('Post' if 'Post' in df_t.columns else None)
            if id_col and pre_col and post_col:
    ...
    # Append bucketing analysis sheets (same content as standalone Bucketing Export).
    if dd_data_path and pre_start_date and pre_end_date and post_start_date and post_end_date:
        try:
            bkt_bytes = _build_bucketing_workbook_bytes(
                dd_data_path,
            ...
            if bkt_bytes:
                bkt_wb = load_workbook(io.BytesIO(bkt_bytes))
                _merge_workbook_sheets(wb, bkt_wb, skip_sheets={"Info"}, sheet_prefix="Bkt ")
        except Exception:
            pass
...
def create_date_export(dd_pre_24_path, dd_post_24_path, dd_pre_25_path, dd_post_25_path,
    ...
    def process_dd_file_for_date_export(file_path, selected_stores):
        ...
        try:
            ...
            if date_col not in df.columns or not store_col or store_col not in df.columns:
                st.warning(
                    f"Missing required columns in {file_path.name}. Looking for '{date_col}' "
                    f"and a store column (Store ID or Merchant store ID). Found: {list(df.columns)[:10]}"
                )
    ...
    def process_ue_file_for_date_export(file_path, selected_stores):
        ...
        try:
            ...
            # For UE files: hardcode to 9th column (index 8) as Order Date — capture name before store-key remap
            if len(df.columns) <= 8:
...
def create_date_export_from_master_files(dd_data_path, ue_data_path, pre_start_date, pre_end_date, post_start_date, post_end_date, excluded_dates=None, operator_name=None):
    ...
    try:
        ...
        def add_dd_sheets(base_sheet_label, pre25_df, post25_df, pre24_df, post24_df):
            dd_store_col = None
            for _ddf in (pre25_df, post25_df, pre24_df, post24_df):
                dd_store_col = get_dd_financial_store_id_column(_ddf)
                if dd_store_col:
                    break

            pre25_sales, pre25_payouts, pre25_orders = _build_period_pivots(pre25_df, 'DD', dd_store_col, 'Subtotal', 'Net total', 'DoorDash order ID')
            post25_sales, post25_payouts, post25_orders = _build_period_pivots(post25_df, 'DD', dd_store_col, 'Subtotal', 'Net total', 'DoorDash order ID')
            pre24_sales, pre24_payouts, pre24_orders = _build_period_pivots(pre24_df, 'DD', dd_store_col, 'Subtotal', 'Net total (for historical reference only)', 'DoorDash order ID')
            post24_sales, post24_payouts, post24_orders = _build_period_pivots(post24_df, 'DD', dd_store_col, 'Subtotal', 'Net total (for historical reference only)', 'DoorDash order ID')
...
def _build_bucketing_agg_tables(csv_path, loader, has_dates,
                                pre_start_date, pre_end_date,
                                post_start_date, post_end_date,
                                excluded_dates=None):
    """
    Run *loader* (DD's load_and_prepare or UE's load_and_prepare_ue) for each window
    and return a fully-loaded context dict for the downstream Excel writer.

    Output dict keys:
        agg_tables_builder(prep, store_operator) → callable to build agg tables (DD/UE pick their own)
        agg_tables       — dict[label -> aggregated DataFrame]
        store_operator   — merged operator/store-name map across all windows
        window_counts    — dict[label -> raw order-row count]
        window_errors    — dict[label -> reason string] for any ValueError raised by loader
        csv_date_min/max — pd.Timestamp.date or None
        csv_total_orders — int
        requested_ranges — dict[label -> (start, end)] of every window we attempted
    """
    def _ts(d):
        if d is None:
            return None
    ...
    def _drop_excluded_dates(prep):
        if prep is None or prep.empty or not excluded_dates or "order_date" not in prep.columns:
            return prep
        ex = {pd.Timestamp(d).normalize().date() for d in excluded_dates}
        out = prep[~prep["order_date"].dt.date.isin(ex)].copy()
        return out

    def _prepare_window(label, start, end):
        try:
            p, so = loader(csv_path, start_date=_ts(start), end_date=_ts(end))
            p = _drop_excluded_dates(p)
            if p is None or p.empty:
                raise ValueError("No orders remain after excluded-date filter.")
            return p, so
        except ValueError as ve:
            window_errors[label] = str(ve)
            return pd.DataFrame(), {}

    csv_date_min = csv_date_max = None
    csv_total_orders = 0
    try:
        _probe, _ = loader(csv_path, start_date=None, end_date=None)
        if _probe is not None and not _probe.empty and "order_date" in _probe.columns:
            _dates = pd.to_datetime(_probe["order_date"], errors="coerce").dropna()
            if not _dates.empty:
                csv_date_min = _dates.min().date()
                csv_date_max = _dates.max().date()
                csv_total_orders = int(len(_probe))
    except Exception:
        pass

    requested_ranges = {
        "Pre": (pre_start_date, pre_end_date),
        "Post": (post_start_date, post_end_date),
    }
    if has_dates:
        ...
        requested_ranges["LY Pre"] = (pre24_s, pre24_e)
        requested_ranges["LY Post"] = (post24_s, post24_e)
        pre25, so_pre25 = _prepare_window("Pre", pre_start_date, pre_end_date)
        post25, so_post25 = _prepare_window("Post", post_start_date, post_end_date)
        pre24, so_pre24 = _prepare_window("LY Pre", pre24_s, pre24_e)
        post24, so_post24 = _prepare_window("LY Post", post24_s, post24_e)
        store_operator = {**so_pre24, **so_post24, **so_pre25, **so_post25}
    else:
        try:
            full, store_operator = loader(csv_path, start_date=None, end_date=None)
            full = _drop_excluded_dates(full)
        except ValueError as ve:
            window_errors["Full"] = str(ve)
            full, store_operator = pd.DataFrame(), {}
        pre25 = post25 = pre24 = post24 = full

    windows = {"Pre": pre25, "Post": post25, "LY Pre": pre24, "LY Post": post24}
    window_counts = {label: (0 if prep is None or prep.empty else int(len(prep)))
    ...
    return {
        ...
        "csv_date_min": csv_date_min,
        "csv_date_max": csv_date_max,
        "csv_total_orders": csv_total_orders,
        "requested_ranges": requested_ranges,
    }


def _write_platform_bucketing_sheets(writer, agg_tables, sheet_suffix=""):
    """
    Write the full bucketing sheet set into *writer* for one platform's agg_tables.
    Sheet names get *sheet_suffix* appended (e.g. " (UE)") to keep DD and UE side-by-side.
    """
    STORE_COL = "Merchant Store ID"

    def _name(base):
        # Excel caps sheet names at 31 chars.
        return (base + sheet_suffix)[:31]
    ...
    if period_rows:
        pd.concat(period_rows, ignore_index=True).to_excel(
            writer, sheet_name=_name("By Period"), index=False)
    ...
    if "Post" in agg_tables and "Pre" in agg_tables:
        ...
        if not post_tp.empty and not pre_tp.empty:
            ...
            ws_gc = writer.book.create_sheet(_name("Daypart GC"))
        ...
        if not post_st.empty and not pre_st.empty:
            ...
            ws_st = writer.book.create_sheet(_name("Store Daypart GC"))
            _write_daypart_gc_2x2_sheet(
                ws_st, post_st, pre_st, delta_st, pct_st, pct_label_cols=2
            )


def _build_bucketing_workbook_bytes(dd_data_path, ue_data_path=None,
                                    pre_start_date=None, pre_end_date=None,
                                    post_start_date=None, post_end_date=None,
                                    excluded_dates=None, *, silent=False):
    """
    Build bucketing analysis workbook bytes.

    When *silent* is True (full-report embed), missing files or empty windows return None
    without Streamlit errors.
    """
    from bucketing_analysis import (
        load_and_prepare, aggregate_slot_table,
        load_and_prepare_ue, aggregate_slot_table_ue,
    )

    dd_csv_path = Path(dd_data_path) if dd_data_path else None
    ue_csv_path = Path(ue_data_path) if ue_data_path else None

    if dd_csv_path is None or not dd_csv_path.exists():
        if not silent:
            st.error(f"DD financial file not found: {dd_csv_path}")
        return None

    has_dates = all([pre_start_date, pre_end_date, post_start_date, post_end_date])

    dd_ctx = _build_bucketing_agg_tables(
        dd_csv_path, load_and_prepare, has_dates,
        pre_start_date, pre_end_date, post_start_date, post_end_date,
        excluded_dates=excluded_dates,
    )
    dd_agg_tables = {
        label: aggregate_slot_table(prep, dd_ctx["store_operator"])
        for label, prep in dd_ctx["windows"].items()
        if prep is not None and not prep.empty
    }

    ue_ctx = None
    ue_agg_tables: dict[str, pd.DataFrame] = {}
    if ue_csv_path is not None and ue_csv_path.exists():
        ue_ctx = _build_bucketing_agg_tables(
            ue_csv_path, load_and_prepare_ue, has_dates,
            pre_start_date, pre_end_date, post_start_date, post_end_date,
            excluded_dates=excluded_dates,
        )
        ue_agg_tables = {
            label: aggregate_slot_table_ue(prep, ue_ctx["store_operator"])
            for label, prep in ue_ctx["windows"].items()
            if prep is not None and not prep.empty
        }
    elif ue_data_path and not silent:
        st.warning(
            f"UE financial file not found at {ue_csv_path}; "
            f"the workbook will only contain DoorDash sheets."
        )

    if not dd_agg_tables and not ue_agg_tables:
        if silent:
            return None
        lines = ["No orders fell inside the selected analysis windows for either platform."]
        for plat_label, ctx in (("DD", dd_ctx), ("UE", ue_ctx)):
            if ctx is None:
                continue
            if ctx["csv_date_min"] and ctx["csv_date_max"]:
                lines.append(
                    f"{plat_label} CSV covers {ctx['csv_date_min']} → {ctx['csv_date_max']} "
                    f"({ctx['csv_total_orders']:,} orders)."
                )
            else:
                lines.append(
                    f"Could not detect any orders in the {plat_label} CSV at all "
                    f"(check the file's date columns)."
                )
            for label, (s, e) in ctx["requested_ranges"].items():
                reason = ctx["window_errors"].get(label, "")
                lines.append(
                    f"  • {plat_label} {label}: {s} → {e}  ({reason or 'no orders matched'})"
                )
        lines.append(
            "Adjust the Pre/Post date window on the Dashboard to overlap the dates present in your CSV(s)."
        )
        raise RuntimeError("\n".join(lines))
    ...
    with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
        ...
        info_ws.cell(row=1, column=1, value="Bucketing Export").font = Font(bold=True, size=14)
        info_ws.cell(row=2, column=1, value=f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        _ir = 4
        info_ws.cell(row=_ir, column=1, value="Platform").font = Font(bold=True)
        info_ws.cell(row=_ir, column=2, value="Window").font = Font(bold=True)
        info_ws.cell(row=_ir, column=3, value="Start").font = Font(bold=True)
        info_ws.cell(row=_ir, column=4, value="End").font = Font(bold=True)
        info_ws.cell(row=_ir, column=5, value="Orders (raw)").font = Font(bold=True)
        _ir += 1

        for plat_label, ctx in (("DD", dd_ctx), ("UE", ue_ctx)):
            if ctx is None:
                continue
            for _lbl, (_s, _e) in ctx["requested_ranges"].items():
                info_ws.cell(row=_ir, column=1, value=plat_label)
                info_ws.cell(row=_ir, column=2, value=_lbl)
                info_ws.cell(row=_ir, column=3, value=str(_s) if _s else "")
                info_ws.cell(row=_ir, column=4, value=str(_e) if _e else "")
                info_ws.cell(row=_ir, column=5, value=ctx["window_counts"].get(_lbl, 0))
                _ir += 1
        ...
        for plat_label, ctx in (("DD", dd_ctx), ("UE", ue_ctx)):
            ...
            if ctx["csv_date_min"] and ctx["csv_date_max"]:
                ...
                info_ws.cell(
                    row=_ir, column=1,
                    value=f"{plat_label} CSV coverage: {ctx['csv_date_min']} → "
                          f"{ctx['csv_date_max']} ({ctx['csv_total_orders']:,} orders)",
                ).font = Font(italic=True)

        if ue_ctx is not None:
            _ir += 2
            info_ws.cell(
                row=_ir, column=1,
                value="UE note: per-order marketing attribution is not present in UE's master export, "
                      "so Mkt Spend / Customer Discounts / Count of Orders Mktg Driven columns are "
                      "omitted from UE sheets.",
            ).font = Font(italic=True)

        for _c, _w in ((1, 10), (2, 12), (3, 14), (4, 14), (5, 14)):
            info_ws.column_dimensions[get_column_letter(_c)].width = _w

        if dd_agg_tables:
            _write_platform_bucketing_sheets(writer, dd_agg_tables, sheet_suffix="")
        if ue_agg_tables:
            _write_platform_bucketing_sheets(writer, ue_agg_tables, sheet_suffix=" (UE)")
...
def create_bucketing_export(dd_data_path, operator_name=None,
                            pre_start_date=None, pre_end_date=None,
                            post_start_date=None, post_end_date=None,
                            excluded_dates=None, ue_data_path=None):
    """
    Run the AITF bucketing analysis on the DoorDash financial file (and the UberEats
    file, if provided) and produce one multi-sheet Excel workbook.

    For each platform present, the workbook contains:
        By Period · By Slot-Period · By Day · Slot Pre vs Post · Slot YoY ·
        Day-Slot Pre vs Post · Day-Slot YoY · Daypart GC · Store Daypart GC

    UE sheets are suffixed " (UE)" to keep them distinguishable. UE does NOT include
    Mkt Spend / Customer Discounts / Count of Orders Mktg Driven — UE's master export
    does not carry per-order marketing attribution (lives in separate UE marketing files).

    Day-part boundaries match the DD bucketing convention for both platforms.

    Returns:
        Tuple of (excel_bytes, filename) or (None, None) on failure.
    """
    try:
        excel_bytes = _build_bucketing_workbook_bytes(
            dd_data_path,
            ue_data_path=ue_data_path,
            pre_start_date=pre_start_date,
            pre_end_date=pre_end_date,
        ...
        filename = (
            f"{tag}_bucketing_export_{timestamp}.xlsx" if tag
