...
def _write_marketing_excel(
    ...
    def _normalize_store_column(df):
        """Rename legacy Merchant store ID columns to Store ID for consistent output."""
        if df is None or not hasattr(df, "columns"):
            return df
        out = df.copy()
        for old in ("Merchant store ID", "Merchant Store ID", "Shop ID"):
            if old in out.columns and old != STORE_ID_COL:
                out = out.rename(columns={old: STORE_ID_COL})
                break
        return out
