...
def format_store_id(store_id):
    """Normalize store IDs for display and consistent string keys."""
    if store_id is None or (isinstance(store_id, float) and pd.isna(store_id)):
        return ""
    if isinstance(store_id, (int, float)) and not isinstance(store_id, bool):
        number = float(store_id)
...
def parse_order_dates(values):
    """Parse order date columns that may use MM/DD/YYYY, MM/DD/YY, or YYYY-MM-DD."""
    original = values.copy()
    parsed = pd.to_datetime(original, format="%m/%d/%Y", errors="coerce")

    missing = parsed.isna()
    if missing.any():
        parsed.loc[missing] = pd.to_datetime(original.loc[missing], format="%m/%d/%y", errors="coerce")
        missing = parsed.isna()

    if missing.any():
        parsed.loc[missing] = pd.to_datetime(original.loc[missing], format="%Y-%m-%d", errors="coerce")
        missing = parsed.isna()

    if missing.any():
        numeric = pd.to_numeric(original.loc[missing], errors="coerce")
        excel_mask = numeric.notna() & numeric.between(20000, 80000)
        if excel_mask.any():
            excel_idx = numeric.loc[excel_mask].index
            parsed.loc[excel_idx] = pd.to_datetime(
                numeric.loc[excel_idx], unit="D", origin="1899-12-30", errors="coerce"
            )
            missing = parsed.isna()

    if missing.any():
        parsed.loc[missing] = pd.to_datetime(original.loc[missing], errors="coerce")
...
def filter_master_file_by_date_range(file_path, start_date, end_date, date_col_name, excluded_dates=None):
    ...
    try:
        ...
        if is_ue_file:
            actual_date_col = detect_ue_date_column(df)
            if actual_date_col is None:
                st.warning(f"UE file {file_path.name} has no recognizable date column. Available columns: {list(df.columns)[:10]}")
                return pd.DataFrame()
        else:
        ...
        if is_ue_file:
            df[actual_date_col] = parse_order_dates(original_dates)
        else:
            df[actual_date_col] = parse_order_dates(original_dates)
