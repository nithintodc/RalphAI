...
def process_master_file_for_ue(file_path, start_date, end_date, excluded_dates=None):
    ...
    try:
        ...
        date_col = detect_ue_date_column(df)
        if date_col is None:
            st.error(f"UE file {file_path.name} has no recognizable date column. Available columns: {list(df.columns)[:10]}")
            return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()

        df[date_col] = parse_order_dates(df[date_col])
