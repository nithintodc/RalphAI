...
from config import ROOT_DIR, DD_DATA_MASTER, UE_DATA_MASTER, SABRRO_DATA_DIR, DD_MKT_PRE_24, DD_MKT_POST_24, DD_MKT_PRE_25, DD_MKT_POST_25, UE_MKT_PRE_24, UE_MKT_POST_24, UE_MKT_PRE_25, UE_MKT_POST_25
from file_upload_screen import resolve_batch_folder
from utils import normalize_store_id_column, filter_excluded_dates, filter_master_file_by_date_range, format_store_id
        ...
        if store_data:
            ...
            for label in ["DoorDash", "UberEats"]:
                ...
                for _, r in sd["worst"].iterrows():
                    g = r['Growth%']
                    sign = "+" if g > 0 else ""
                    h.append('<span class="ki-tag down">#{} {}{:.1f}%</span>'.format(format_store_id(r['Store ID']), sign, g))
                h.append('</div></div>')
                h.append('<div class="ki-sc up"><div class="sc-title">&#9650; {} growing</div><div class="ki-tag-list">'.format(label))
                for _, r in sd["best"].iterrows():
                    g = r['Growth%']
                    sign = "+" if g > 0 else ""
                    h.append('<span class="ki-tag up">#{} {}{:.1f}%</span>'.format(format_store_id(r['Store ID']), sign, g))
                h.append('</div></div>')
            h.append('</div></div>')
...
def _resolve_file_paths():
    ...
    if SABRRO_DATA_DIR.is_dir() and any(SABRRO_DATA_DIR.glob("*.csv")):
        ...
        if dd_data_path is None and batch.get("dd_financial"):
            ...
            st.info(f"Using DoorDash financial from sabrro_data: {dd_data_path.name}")
        if ue_data_path is None and batch.get("ue_financial"):
            ue_data_path = batch["ue_financial"]
            st.info(f"Using UberEats financial from sabrro_data: {ue_data_path.name}")
        if marketing_folder_path is None and batch.get("marketing_folder"):
            marketing_folder_path = batch["marketing_folder"]
            st.info(f"Using DoorDash marketing files from sabrro_data/")
...
def _fmt_summary_table(summary_df, is_yoy=False):
    """Format a summary DataFrame for display (currency/percentage/integer formatting)."""
    display = summary_df.copy()
    for col in display.columns:
        display[col] = display[col].astype(object)

    if is_yoy:
        value_cols = ['last year-post', 'post', 'YoY']
        pct_col = 'YoY%'
    else:
        value_cols = ['Pre', 'Post', 'PrevsPost', 'LastYear Pre vs Post']
        pct_col = 'Growth%'
    ...
    for idx in display.index:
        ...
        for col in value_cols:
            ...
            if metric in ('Orders', 'New Customers'):
                display.loc[idx, col] = f"{int(round(val)):,}"
            elif metric == 'Profitability':
                display.loc[idx, col] = f"{val:.1f}%"
            elif metric == 'Average Check':
                display.loc[idx, col] = f"${val:,.1f}"
            else:
                display.loc[idx, col] = f"${val:,.1f}"
        if pct_col in display.columns:
            display.loc[idx, pct_col] = f"{summary_df.loc[idx, pct_col]:.1f}%"

    for col in display.columns:
        display[col] = display[col].astype(str)

    if 'LastYear Pre vs Post' in display.columns:
        display = display.rename(columns={'LastYear Pre vs Post': 'LY Pre/Post'})
    if 'last year-post' in display.columns:
        display = display.rename(columns={'last year-post': 'LY Post'})
    if 'post' in display.columns and is_yoy:
        display = display.rename(columns={'post': 'Post'})

    return display


def _fmt_store_table_prepost(tbl):
    """Format combined store table (Pre vs Post) for display."""
    display = tbl.reset_index() if tbl.index.name == 'Store ID' else tbl.copy()
    if 'Pre' in display.columns and 'Post' in display.columns:
        display = display[
            (display['Store ID'].notna() if 'Store ID' in display.columns else True) &
            (display['Store ID'] != '' if 'Store ID' in display.columns else True) &
            ((display['Pre'].fillna(0) != 0) | (display['Post'].fillna(0) != 0))
        ].copy()
    if display.empty:
        return display
    display = display.reset_index(drop=True)
    for col in ['Pre', 'Post', 'PrevsPost']:
        if col in display.columns:
            display[col] = display[col].apply(lambda x: f"${x:,.1f}" if isinstance(x, (int, float)) else x)
    if 'LastYear Pre vs Post' in display.columns:
        display['LY Pre/Post'] = display['LastYear Pre vs Post'].apply(lambda x: f"${x:,.1f}" if isinstance(x, (int, float)) else x)
        display = display.drop(columns=['LastYear Pre vs Post'])
    if 'Growth%' in display.columns:
        display['Growth%'] = display['Growth%'].apply(lambda x: f"{x:.1f}%" if isinstance(x, (int, float)) else x)
    if 'Store ID' in display.columns:
        display = display.set_index('Store ID')
    return display


def _fmt_store_table_yoy(tbl):
    """Format combined store table (YoY) for display."""
    display = tbl.reset_index() if tbl.index.name == 'Store ID' else tbl.copy()
    if 'last year-post' in display.columns and 'post' in display.columns:
        display = display[
            (display['Store ID'].notna() if 'Store ID' in display.columns else True) &
            (display['Store ID'] != '' if 'Store ID' in display.columns else True) &
            ((display['last year-post'].fillna(0) != 0) | (display['post'].fillna(0) != 0))
        ].copy()
    if display.empty:
        return display
    display = display.reset_index(drop=True)
    if 'last year-post' in display.columns:
        display['LY Post'] = display['last year-post'].apply(lambda x: f"${x:,.1f}" if isinstance(x, (int, float)) else x)
        display = display.drop(columns=['last year-post'])
    if 'post' in display.columns:
        display['Post'] = display['post'].apply(lambda x: f"${x:,.1f}" if isinstance(x, (int, float)) else x)
        display = display.drop(columns=['post'])
    if 'YoY' in display.columns:
        display['YoY'] = display['YoY'].apply(lambda x: f"${x:,.1f}" if isinstance(x, (int, float)) else x)
    if 'YoY%' in display.columns:
        display['YoY%'] = display['YoY%'].apply(lambda x: f"{x:.1f}%" if isinstance(x, (int, float)) else x)
    if 'Store ID' in display.columns:
        display = display.set_index('Store ID')
...
def main():
    ...
    with store_tab:
        render_section_header("Combined Store-Level Sales", "Both platforms rolled up at store level.", ("Combined", "info"))
        left_col, right_col = st.columns(2)
        with left_col:
            if combined_store_table1 is not None and not combined_store_table1.empty:
                st.subheader("Pre vs Post")
                display = _fmt_store_table_prepost(combined_store_table1)
                if not display.empty:
                    st.dataframe(style_signed_table(display), use_container_width=True, height=290)
                else:
                    st.info("No data available for Combined Table 1")
            else:
                st.info("No combined store data available.")
        with right_col:
            if combined_store_table2 is not None and not combined_store_table2.empty:
                st.subheader("Year over Year")
                display = _fmt_store_table_yoy(combined_store_table2)
                if not display.empty:
                    st.dataframe(style_signed_table(display), use_container_width=True, height=290)
                else:
                    st.info("No data available for Combined Table 2")
            else:
                st.info("No combined YoY data available.")
    ...
    with summary_tab:
        render_section_header("Combined Summary Analysis", "Platform totals across sales, payouts, orders, and customers.", ("Combined", "info"))
        combined_sum_left, combined_sum_right = st.columns(2)
        with combined_sum_left:
            st.write("**Pre vs Post Analysis**")
            st.dataframe(style_signed_table(_fmt_summary_table(combined_summary1, is_yoy=False)), use_container_width=True)
        with combined_sum_right:
            st.write("**Year-over-Year Analysis**")
            st.dataframe(style_signed_table(_fmt_summary_table(combined_summary2, is_yoy=True)), use_container_width=True)
    ...
    with marketing_tab:
        render_section_header("Corporate vs TODC Marketing", "Post-period promotion and sponsored-listing spend split.")
            ...
            with col_promo:
                with st.expander("Promotion Details", expanded=False):
                    if not promotion_table.empty:
                        st.dataframe(style_signed_table(_fmt_corporate_display(promotion_table)), use_container_width=True)
                    else:
                        st.info("No promotion data available.")
            with col_spons:
                with st.expander("Sponsored Listing Details", expanded=False):
                    if not sponsored_table.empty:
                        st.dataframe(style_signed_table(_fmt_corporate_display(sponsored_table)), use_container_width=True)
                    else:
