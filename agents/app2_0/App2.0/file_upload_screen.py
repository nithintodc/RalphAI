...
from utils import parse_order_dates as _parse_date_series
...
def sniff_category_from_header(header_sample):
    """
    Detect DoorDash vs UberEats financial exports from the CSV header row.
    Returns 'dd_financial', 'ue_financial', or None if inconclusive.
    """
    if not header_sample:
        return None
    first_line = header_sample.splitlines()[0].upper() if header_sample else ""
    if not first_line:
        return None
    if "MARKETING_PROMOTION" in first_line or "MARKETING_SPONSORED" in first_line:
        return None
    if "DOORDASH" in first_line or "TIMESTAMP LOCAL DATE" in first_line:
        return "dd_financial"
    ue_markers = (
        "LOCAL DATE THE ORDER WAS PLACED",
        "SALES (EXCL. TAX)",
        "UBER EATS",
        "EXTERNAL STORE ID AS PER UBER EATS",
    )
    if any(marker in first_line for marker in ue_markers):
        return "ue_financial"
    return None


def classify_file(filename, header_sample=None):
    """
    Classify a file based on its filename pattern (and optional CSV header sniff).
    Returns a category key from FILE_CATEGORIES.

    Rules (on basename, case-insensitive):
      *MARKETING_SPONSORED* -> dd_ads
      *MARKETING_PROMO*     -> dd_promo (includes MARKETING_PROMOTION exports)
      FINANCIAL*            -> dd_financial (prefix only)
      everything else       -> ue_financial, unless header sniff says DoorDash financial
    """
    upper = Path(filename).name.upper()
    if "MARKETING_SPONSORED" in upper:
        return "dd_ads"
    if "MARKETING_PROMO" in upper:
        return "dd_promo"
    if upper.startswith("FINANCIAL"):
        return "dd_financial"

    sniffed = sniff_category_from_header(header_sample)
    if sniffed:
        return sniffed
    return "ue_financial"


def read_upload_header_sample(uploaded_file, nbytes=8192):
    """Read the first bytes of an uploaded file for header-based classification."""
    pos = uploaded_file.tell()
    uploaded_file.seek(0)
    raw = uploaded_file.read(nbytes)
    uploaded_file.seek(pos)
    return raw.decode("utf-8", errors="replace")


def classify_upload(uploaded_file):
    """Classify a Streamlit uploaded file using filename + header sniff."""
    return classify_file(uploaded_file.name, read_upload_header_sample(uploaded_file))


def resolve_batch_folder(folder_path):
    """
    Classify all CSVs in a folder and return canonical paths for the analytics app.

    Returns:
        dict with optional keys: dd_financial, ue_financial, marketing_folder (Path)
    """
    folder = Path(folder_path)
    if not folder.is_dir():
        return {}

    classified = {key: [] for key in CATEGORY_ORDER}
    for path in sorted(folder.glob("*.csv")):
        try:
            sample = path.read_text(encoding="utf-8", errors="replace")[:8192]
        except OSError:
            continue
        classified[classify_file(path.name, header_sample=sample)].append(path)

    resolved = {}
    if classified["dd_financial"]:
        resolved["dd_financial"] = classified["dd_financial"][0]
    if classified["ue_financial"]:
        resolved["ue_financial"] = classified["ue_financial"][0]
    marketing_files = classified["dd_promo"] + classified["dd_ads"]
    if marketing_files:
        resolved["marketing_folder"] = folder
    return resolved
...
.mapping-rule .count {
    ...
    if pending_uploads:
        ...
        for pending_file in pending_uploads:
            pending_counts[classify_upload(pending_file)] += 1
    ...
    if all_files and len(all_files) > 0:
        ...
        if marketing_files_all:
            ...
            for uploaded_file in marketing_files_all:
                category = classify_upload(uploaded_file)
