...
| **`Reporting-browser-use-claude-code/`** | Used by Offers/Ads flows from the API (subprocess). Must be **in the container / VM filesystem** next to the app (repo layout already does this). |
...
> **Note:** `.dockerignore` excludes `venv`, `dashboard/node_modules`, and `data/runs/**`. The full repo (including `Reporting-browser-use-claude-code/`) is still large; keep images updated only when you change dependencies or code.
...
- Use a **remote browser** (e.g. **Browserless**, or your existing **CDP** pattern in `LOCAL_BROWSER_CDP_URL` from `Reporting-browser-use-claude-code`) and document env vars in Secret Manager.
...
For a **VM-oriented** all-in-one install, you can also adapt patterns from `Reporting-browser-use-claude-code/GCP_VM_SETUP_ALL_IN_ONE.md` (same org, similar browser constraints).

---

## Support checklist if something fails

1. **Cloud Run logs** — import errors, missing `PYTHONPATH`, missing `Reporting-browser-use-claude-code`.
